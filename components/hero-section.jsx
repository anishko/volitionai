...
const HeroSection = ({
    ...
    useEffect(() => {
        // Resolve --foreground from inside our own subtree so the arrow color
        // follows the theme this hero is placed in (e.g. a `dark` wrapper),
        // not the document root — otherwise a dark arrow vanishes on a dark hero.
        const host = rootRef.current || document.body;
        const tempElement = document.createElement('div');
        tempElement.style.display = 'none';
        host.appendChild(tempElement);
    ...
    const ctaClassName = "py-2 px-4 rounded-xl border border-foreground/50 hover:border-foreground/80 text-foreground transition-colors focus:outline-none focus:ring-2 focus:ring-ring";

    const hasMedia = Boolean(imageUrl || videoUrl);

    return (
        <div ref={rootRef} className="bg-background text-foreground min-h-screen flex flex-col">
            ...
            <main className="flex-grow flex flex-col items-center justify-center">
                ...
                <div className="mt-8 flex justify-center">
                    {buttonHref ? (
                        <a ref={targetRef} href={buttonHref} className={ctaClassName}>
                            {buttonText}
                        </a>
                    ) : (
                        <button ref={targetRef} type="button" onClick={onButtonClick} className={ctaClassName}>
                            {buttonText}
                        </button>
                    )}
                </div>
                ...
                <div className="mt-12 lg:mt-16 w-full max-w-screen-sm mx-auto overflow-hidden px-4 sm:px-2">
                    <div className="bg-border rounded-[2rem] p-[0.25rem]">
                        <div className={`relative rounded-[1.75rem] bg-card overflow-hidden ${hasMedia ? 'h-64 sm:h-72 md:h-80 lg:h-96 flex items-center justify-center' : ''}`}>
                            {imageUrl && (
                                // eslint-disable-next-line @next/next/no-img-element -- accepts arbitrary external preview URLs
                                <img
                                    src={imageUrl}
                                    alt="Preview"
                            ...
                            {!hasMedia && (
                                placeholder ?? (
                                    <div className="flex h-64 items-center justify-center text-muted-foreground italic sm:h-72 md:h-80 lg:h-96">
                                        Card Content Area
                                    </div>
                                )
                            )}
